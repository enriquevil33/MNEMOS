    ...
    <div *ngIf="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div class="bg-panel border border-divider rounded-xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden animate-fade-in">
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar">
          ...
          <!-- ========== Details Tab ========== -->
          <div *ngIf="activeTab === 'details'" class="p-6 space-y-4">

            <div *ngIf="errorMessage" class="p-3 bg-error/10 border border-error/30 rounded-lg text-error text-sm">
              {{ errorMessage }}
            </div>
            ...
            <div>
              ...
              <input
                ...
                class="w-full px-3 py-2 rounded-lg border border-divider bg-base-100 text-primary text-sm focus:outline-none focus:ring-2 focus:ring-accent/50 disabled:opacity-50"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows="3"
                class="w-full px-3 py-2 rounded-lg border border-divider bg-base-100 text-primary text-sm focus:outline-none focus:ring-2 focus:ring-accent/50 disabled:opacity-50 resize-none"
              ></textarea>
            </div>
          </div>

          <!-- ========== Documents Tab ========== -->
          <div *ngIf="activeTab === 'documents'" class="p-6 space-y-4">
            ...
            <div class="space-y-4">
              ...
              <div>
                ...
                <div *ngIf="collectionDocs().length === 0" class="text-secondary text-sm text-center py-3 border border-dashed border-divider rounded-lg">
                  No documents yet
                </div>
                <div class="space-y-1.5">
                  <div *ngFor="let doc of collectionDocs()"
                       class="flex items-center justify-between px-3 py-2 rounded-lg border border-divider bg-base-200">
                    <div class="flex items-center gap-2.5 overflow-hidden">
                      ...
                      <span *ngIf="doc.status !== 'completed'" class="text-xs text-secondary shrink-0">({{ doc.status }})</span>
                    </div>
              ...
              <div>
                ...
                <div class="flex items-center gap-2 px-3 py-2 bg-input border border-divider rounded-lg focus-within:ring-2 focus-within:ring-accent/50 mb-2">
                  ...
                  />
                  <button *ngIf="addSearch" (click)="addSearch = ''" class="text-secondary hover:text-primary transition-colors shrink-0">
                ...
                <div class="max-h-48 overflow-y-auto custom-scrollbar rounded-lg border border-divider">
                  <div *ngIf="filteredAvailable().length === 0" class="text-secondary text-sm text-center py-3">
                    {{ addSearch ? 'No matches' : 'All documents are in this collection' }}
                  </div>
                  <button
                    *ngFor="let doc of filteredAvailable()"
